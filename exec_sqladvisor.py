...
def get_conf(dbname, db_info):
    ...
    if len(str(base64.b64decode(db_info)).split(',,')) < 5:
        print('db_info error')
        exit(-100)

    db_mark, db_host, db_port, db_user, db_pwd = str(base64.b64decode(db_info)).split(',,')
    if db_pwd == 'null':
        db_pwd = ''
...
def exec_sqladvisor(mark, db_info, dbname, sqls):
    print('exec db is {}'.format(mark))
    conf_file = get_conf(dbname, db_info)
    for s in sqls.split(';'):
        if s:
